...
# helper functions
function script:coalesce($a, $b) { if($a) { return $a } $b }
...
function script:appsdir($global) { "$(basedir $global)\apps" }
function shimdir($global) { "$(basedir $global)\shims" }
function script:appdir($app, $global) { "$(appsdir $global)\$app" }
function script:versiondir($app, $version, $global) { "$(appdir $app $global)\$version" }
...
function script:env($name,$global,$val='__get') {
...
function shim(
    ...
    echo ("{0}{1}{2}" -f '$path = ', $prefix, "`"$relpath`"") | out-file $shim -encoding utf8
    if($arg) {
...
#Export-ModuleMember -Function is_admin, abort, warn, error, success, basedir, appsdir, shimdir, 
